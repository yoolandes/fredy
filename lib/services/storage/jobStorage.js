...
export const upsertJob = ({
  ...
  if (existing) {
    ...
    SqliteConnection.execute(
      `INSERT INTO jobs (id, user_id, enabled, name, blacklist, provider, notification_adapter, shared_with_user, spatial_filter, spec_filter, interval_minutes)
       VALUES (@id, @user_id, @enabled, @name, @blacklist, @provider, @notification_adapter, @shareWithUsers, @spatialFilter, @specFilter, @intervalMinutes)`,
      {
        id,
        user_id: ownerId,
        enabled: enabled ? 1 : 0,
...
export const getJob = (jobId) => {
  const row = SqliteConnection.query(
    `SELECT j.id,
            ...
            j.interval_minutes AS intervalMinutes,
